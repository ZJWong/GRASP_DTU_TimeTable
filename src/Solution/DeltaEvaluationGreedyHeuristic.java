...
public class DeltaEvaluationGreedyHeuristic {
...
//	// Define objects present in the objective function
	...
	public static double getTotalAfterChangeCost(ArrayList<Schedule[][]> solution, int unscheduledCoursesBeforeMove, 
			int uncheduledCoursesAfterMove, ArrayList<Course> unscheduledCourses, ArrayList<ArrayList<TimeSlot>> coursePositions,
		...
		curriculumCompactnessPenalty = getTotalCurriculumCompactnessPenalty(solution, course, scheduleIndex, dayIndex, periodIndex);
	 ...
	 * @param coursePositions ArrayList of course positions.
	 ...
	 * @return
	 */
	public static double getTotalCurriculumCompactnessPenalty(ArrayList<Schedule[][]> solution, Course course, int scheduleIndex, int dayIndex, int periodIndex) {
...
//			outputFile.println("calling curr. compactness case 1");
			...
			totalAfterChangePenalty += curriculumCompactnessOfCourseAtPosition(solution, scheduleIndex, course, dayIndex, periodIndex);
			totalAfterChangePenalty += curriculumCompactnessOfCourseAtPosition(solution, scheduleIndex, course, dayIndex, periodIndex + 1);
		}
...
//			outputFile.println("calling curr. compactness case 3");
	...
	public static double curriculumCompactnessOfCourseAtPosition(ArrayList<Schedule[][]> solution, int scheduleIndex, Course course, int dayIndex, int periodIndex) {
...
//	/**
//	 * Calculates the change in room stability penalty
//	 * @param coursePositions ArrayList of courses and their current scheduled positions in solution
//	 * @param course Current course added/removed.
//	 * @return roomStabilityPenaltyChange
//	 */
...
//		
//		for (int p = 0; p < coursePositions.get(courseNumber).size(); p++) {
//			
//			if (coursePositions.get(courseNumber).get(p).getRoomId().equals("")) continue;
//			
//			if (p == 0) {
//				// Add first room to roomsUsed
//				roomsUsed.add(coursePositions.get(courseNumber).get(p).getRoomId());
//			}
//			
//			else if (p >= 1) {
//				
//				for (int s = 0; s < roomsUsed.size(); s++) {
//					// Determine whether the room id is already included in RoomsUsed
//					if (roomsUsed.get(s).equals((coursePositions.get(courseNumber).get(p).getRoomId()))) {
//						included = true;
//					}
//					
//					// If not add the room to arrayList
//					if (!included) {						
//						roomsUsed.add(coursePositions.get(courseNumber).get(p).getRoomId());
//						included = false;
//					}
...
//		}
//		
//		// If more than 1 room assigned -> apply penalty
//		if (roomsUsed.size() > 1) {
//			roomStabilityPenalty = ( roomsUsed.size()-1 ) * roomStabilityPenaltyFactor;
//
//		}
//		
